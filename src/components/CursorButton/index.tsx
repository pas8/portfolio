...
const CursorButton: FC<{ title: string; onClick?: (e?: any) => void,href?:string;customIcon?:any }> = ({ title, onClick ,href,customIcon =false}) => {
  const { button } = useLocalStyles();
  return (
    <CursorContext.Consumer>
      {({ mouseOutEvent, mouseOverEvent }) => (
        //@ts-ignore
        <ButtonBase onClick={onClick} className={button} onMouseOver={mouseOverEvent} onMouseOut={mouseOutEvent} href={href}>
          <Grid container justifyContent={'center'} alignItems={'center'} className={'content'}>
     {  customIcon ||    <svg viewBox={'0 0 52 52'} xmlns="http://www.w3.org/2000/svg">
