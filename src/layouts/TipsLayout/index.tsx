...
if(isLastStep) return setTimeout(() => {
  onClose()
}, 8000);
},[isLastStep])
  ...
  const H = boundingClientRect?.height;
      ...
      <Zoom in={!isTipsLayoutHidden}>
        ...
        <Grid className={'confitiContainer'}>
         ...
         {  step === 0 &&    <CursorButton title={'Start'} onClick={toNextStep}/>}
          ...
          {isLastStep && (
            <Grid container alignItems={'center'} justifyContent={'center'}className={isLastStep && 'succssesSvgContainer'} >
...
{step !== 0 && <Grid className={'backArrowButtonContainer'}>
