import { makeStyles, Grid, MenuItem, Typography, SvgIcon, IconButton } from '@material-ui/core';
...
const useLocalStyles = makeStyles(({ palette: { background, secondary, text, primary }, breakpoints }) => ({
  mainContainer: {
    position: 'relative'
  },
  navContainer: {
...
const NavLayout: FC = ({ children }) => {
  const { navContainer, mainContainer } = useLocalStyles();
  ...
  const pathsPropertyiesArr = [
    { className: clsx(duration16Sec, dashOffset1000Dasharray1000, deepPink2Color) },
    { className: clsx(delayOSec, dashOffset2000Dasharray500, springGreen3Color) },
    { className: clsx(duration24Sec, dashOffset1500Dasharray6000, lime2TurquoiseColor) },
    { className: clsx(duration32Sec, dashOffset2050Dasharray250, steelBlue3Color) }
  ];
  ...
  return (
    <Grid>
      <Grid className={navContainer} container direction={'column'} justifyContent={'space-around'}alignItems={'center'} style={{ width: 142 }}>
        <Grid className={'logoContainer'} container justifyContent={'center'}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 800" className={'svgContainer'}>
            {pathsPropertyiesArr.map((props, idx) => {
              return (
                <Fragment key={`pathsPropertyiesArr_${idx}`}>
                  {paths.map((path, componentIdx) => (
                    <path key={`pathsPropertyiesArr_${idx}_${componentIdx}`} d={path} {...props}></path>
                  ))}
                </Fragment>
              );
            })}
        ...
        <Grid className={'navFooterContainer'}>
          <IconButton>
